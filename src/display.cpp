...
void forceRedisplay() { displayNeedsUpdate = true; }
...
void displayDynamicData() {
    ...
    lcd.setCursor(9, 3);
    for (int i = 0; i < 6; i++) lcd.print(getRelayState(i) ? '1' : '0');
...
void handleMenu() { enterMenu(); }

/// Отображение меню (обёртка)
void displayMenu() { runMenu(mainMenu, mainMenuLength, mkRoot); }
...
void displayFilterWashInfo() {
    ...
    if (needRedraw) {
        ...
        switch (filterWashScrollOffset) {
            case 0:
                displayFilterWashScreen1();
                break;
            case 1:
                displayFilterWashScreen2();
                break;
            case 2:
                displayFilterWashScreen3();
                break;
            case 3:
                displayFilterWashScreen4();
                break;
        }
...
int getSelectedMenuItem() { return getSelectedMenuItemIndex(); }
