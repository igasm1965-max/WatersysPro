...
void displayFilterWashInfo() {
  ...
  // Нажатие кнопки — выход из инфо режима (дебаунс)
  if (isEncoderButtonPressed() && (currentTime - lastEncoderButtonTime) > DEBOUNCE_MS) {
  ...
  bool needRedraw = pageChanged || (millis() - lastUpdate >= UPDATE_INTERVAL_MS);
...
void displayBackwashScreen() {
  ...
  // Если обнаружена ошибка сухого хода - показываем ошибку
  if (flags.backwashDryRunError) {
    ...
    if (millis() - lastUpdate >= 500) {
      ...
      lcd.setCursor(0, 0);
      lcd.print("!!! ERROR: DRY RUN !");

      lcd.setCursor(0, 1);
      lcd.print("Filter water level");

      lcd.setCursor(0, 2);
      lcd.print("too low during WaSH ");

      // Нижняя строка: уровень бака 2 (мигает)
      lcd.setCursor(0, 3);
      if (flags.blinkState) {
        lcd.print("Tank2:");
        printTwoDigits(tank2Level);
