...
bool isEngineerModeUnlocked() { return engineerModeUnlocked; }

void lockEngineerMode() { engineerModeUnlocked = false; }
 ...
 */
static void editValue(const char* title, uint16_t* value, uint16_t minVal, uint16_t maxVal,
                      uint16_t step, const char* unit) {
...
void showEventLogSettings() {
    ...
    const char* labels[] = {"Wi-Fi events", "Manual ops", "Emergency", "Watchdog",   "Sensor Err",
                            "Settings",     "Web/API",    "State Chg", "Errors only"};
    const uint32_t bits[] = {LOG_FILTER_WIFI,     LOG_FILTER_MANUAL,      LOG_FILTER_EMERGENCY,
                             LOG_FILTER_WATCHDOG, LOG_FILTER_SENSOR,      LOG_FILTER_SETTINGS,
                             LOG_FILTER_WEB,      LOG_FILTER_STATECHANGE, LOG_FILTER_ERRORS};
    const int numOptions = sizeof(bits) / sizeof(bits[0]);

    uint32_t mask =
        preferences.getULong(PREF_KEY_LOG_FILTER, (uint32_t)PREF_KEY_LOG_FILTER_DEFAULT);
    int selected = 0;
    bool exit = false;
    ...
    while (!exit) {
        ...
        if (delta != 0) {
            if (delta < 0 && selected > 0)
                selected--;
            else if (delta > 0 && selected < numOptions - 1)
                selected++;
            needUpdate = true;
        }
...
static void editString(const char* title, const char* prefKey, int maxLen, bool isPassword) {
    ...
    while (editing) {
        ...
        if (action == eRight) {
            if (cursorMode) {
                ...
                if (pos < value.length())
                    pos++;
                else if (value.length() < effectiveMaxLen)
                    pos++;
            } else {
                // change current char
        ...
        if (isEncoderButtonPressed()) {
            ...
            if (pos < value.length())
                value.setCharAt(pos, c);
            else
                value += c;
            pos++;
            if (pos >= effectiveMaxLen) {
                editing = false;
                break;
            }
            charIdx = 0;
        ...
        if ((millis() - lastDraw >= 80) &&
            (lastDisp != value || lastPos != pos || lastCharIdx != charIdx ||
             cursorMode != (lastCharIdx == -2))) {
            lcd.clear();
            lcd.setCursor(0, 0);
            char tbuf[21];
            strncpy(tbuf, title, 20);
    ...
    // If editing finished normally (not via long press save we already did it), then save trimmed
    // value
    if (!isEncoderButtonPressed()) {
        if (value.length() > 0) {
            preferences.putString(prefKey, value);
        ...
        if (action == eRight)
            ...
            switch (selected) {
                case 0:
                    ...
                    // flush encoder/button state left by editor and wait release to avoid "stuck"
                    // UI
                    readEncoderDelta();
                    wasEncoderButtonPressed();
                    waitForEncoderRelease();
...
void showMqttSetup() {
    flags.displayLocked = 1;
    const char* items[] = {
        "Enable/Disable", "Broker",     "Port",      "TLS Enable", "TLS Port",
        "User",           "Pass",       "Client ID", "Topic base", "Interval (s)",
        "Test publish",   "Clear MQTT", "Back"};
    int itemCount = 13;
    int selected = 0;
    bool exitMenu = false;
    int lastSelected = -1;
...
void editMqttClientId() {
    editString("MQTT Client ID", PREF_KEY_MQTT_CLIENT_ID, 64, false);
    initVQTT();
}
...
void editWiFiSSID() { editString("Edit SSID", PREF_KEY_WIFI_SSID, 32, false); }

void editWiFiPassword() { editString("Edit PASS", PREF_KEY_WIFI_PASS, 63, true); }
        ...
        if (page != lastPage) {
            ...
            switch (page) {
                ...
                case 1:
                    ...
                    lcd.setCursor(0, 2);
                    lcd.print("PumpDry:");
                    lcd.print(safetySettings.pumpDryTimeoutSeconds);
                    lcd.print(" s");
                    lcd.setCursor(0, 3);
                    lcd.print("Poll:");
                    lcd.print(safetySettings.sensorPollPeriod * 0.1f, 1);
                    lcd.print(" s");
                    break;
...
void drawPage2() {
    ...
    char macBuf[18];
    snprintf(macBuf, sizeof(macBuf), "%02X:%02X:%02X:%02X:%02X:%02X", macBytes[0], macBytes[1],
...
void loadSafetySettings() {
    ...
    safetySettings.pumpDryTimeoutSeconds =
        preferences.getUShort("sf_pump_to", DEFAULT_PUMP_DRY_TIMEOUT_SECONDS);
    safetySettings.pumpMinLevelDeltaCm =
        preferences.getUChar("sf_pump_delta", DEFAULT_PUMP_MIN_LEVEL_DELTA_CM);
    safetySettings.pumpDryConsecutiveChecks =
        preferences.getUChar("sf_pump_cons", DEFAULT_PUMP_DRY_CONSECUTIVE_CHECKS);

    // Sensor poll period (0.1s units). Range 1..20 corresponds to 0.1..2.0 seconds.
    // Legacy migration removed (previously multiplied values 1..10 by 10).
    // Old code:
    ...
    while (true) {
        ...
        if (action == eRight) {
            ...
            if (newTimeZoneOffset != oldTimeZoneOffset) {
                ...
                // Получаем текущее время из RTC (которое сейчас хранит локальное время в старом
                // поясе)
            ...
            saveEventLog(
                LOG_INFO, EVENT_TIMEZONE_CHANGED,
                safetySettings.timeZoneOffset + 128);  // +128 для сохранения отрицательных значений
