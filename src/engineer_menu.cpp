...
void saveSafetySettings() {
  ...
  // ============ ДАТЧИКИ И ВСЕ ОСТАЛЬНЫЕ ПАРАМЕТРЫ ============
  preferences.putUShort("sf_sen_poll", safetySettings.sensorPollPeriod);
  
  // ============ ВРЕМЕННАЯ ЗОНА (используем централизованный ключ) ============
  extern Preferences preferences;
  preferences.putChar(PREF_KEY_TIMEZONE, safetySettings.timeZoneOffset);
  
  Serial.println("[PREFS] Safety settings saved successfully");
}
...
void loadSafetySettings() {
  ...
  Serial.println("[PREFS] Safety settings loaded successfully");
}
