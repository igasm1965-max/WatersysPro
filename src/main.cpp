 ...
 * - Управления через энкодер с LCD меню (20x4)
 * - Мониторинга уровня воды через УЗ датчики
...
extern void saveRuntimeState();
extern bool loadRuntimeState();
...
/// Система Preferences для ESP32 (вместо EEPROM)
Preferences preferences;

/// LCD дисплей (20x4 на адресе 0x27) — теперь обёртка SafeLCD
...
/// Параметры бака 1
TankParams tank1 = {10, 80};
/// Параметры бака 2
TankParams tank2 = {10, 80};
/// Текущий уровень бака 1 (см)
int tank1Level = 0;
/// Текущий уровень бака 2 (см)
int tank2Level = 0;
/// Текущий уровень бака 1 (см, измеренный с помощью УЗ датчика)
int tank1RawDistance = 0;
/// Текущий уровень бака 2 (см, измеренный с помощью УЗ датчика)
int tank2RawDistance = 0;

// ============ СТАТИСТИКА WATCHDOG ============

/// Статистика сбросов WDT
WDTStats wdtStats;
...
bool engineerModeUnlocked = false;
...
uint32_t currentAerationRemaining = 0;
uint32_t currentOzonationRemaining = 0;
...
EventLog eventLogs[200];
/// Индекс текущего события в циклическом буфере
uint8_t eventLogIndex = 0;

// ============ ПЕРЕМЕННЫЕ ДЛЯ ЭНКОДЕРА ============

// encoderPos, encoderButtonPressed, lastEncoderInterrupt объявлены в encoder.cpp
...
unsigned long menuIdleTimerStart = 0;
unsigned long lastEncoderCheck = 0;
unsigned long lastEncoderRotationTime = 0;  ///< Дребезг вращения энкодера
unsigned long lastEncoderButtonTime = 0;    ///< Дребезг кнопки энкодера
int filterWashScrollOffset = 0;
int eventListScrollOffset = 0;
int eventDetailPage = 0;
int currentFilterType = 0;

// ============ ПЕРЕМЕННЫЕ АВАРИЙНОГО РЕЖИМА ============

char emergencyMessage[41] = "";
unsigned long emergencyLastBlinkTime = 0;
unsigned long emergencyLastRuntimeUpdate = 0;

// ============ ПЕРЕМЕННЫЕ ДЛЯ ВОССТАНОВЛЕНИЯ ПОСЛЕ МЕНЮ ============

SystemState stateBeforeMenu = STATE_IDLE;
unsigned long stateTimeBeforeMenu = 0;
int tank1LevelBeforeMenu = 0;
int tank2LevelBeforeMenu = 0;
bool tank1EmptyBeforeMenu = false;
bool tank2EmptyBeforeMenu = false;
bool waterTreatmentWasInProgress = false;
bool filtrationWasInProgress = false;
bool backwashWasInProgress = false;
uint32_t remainingOzonationBeforeMenu = 0;
uint32_t remainingAerationBeforeMenu = 0;
uint32_t remainingSetlingBeforeMenu = 0;

// ============ НОЧНОЙ РЕЖИМ ============

bool nightPauseActive = false;
bool timeoutWasSaved = false;
unsigned long savedBacklightTimeout = 0;
unsigned long backlightTimeout = 5 * 60 * 1000UL;

// ============ СЧЕТЧИКИ И ТАЙМЕРЫ ============

unsigned long lastBlinkUpdate = 0;
unsigned long lastEmergencyUpdate = 0;
unsigned long stateUpdateTimerLast = 0;
unsigned long displayUpdateTimerLast = 0;
unsigned long sensorPollTimerLast = 0;
unsigned long emergencyCheckTimerLast = 0;
unsigned long wdtStatsSaveTimerLast = 0;
unsigned long blinkTimerLast = 0;
unsigned long countdownTimerLast = 0;  // Отдельный таймер для updateCountdownTimers()
unsigned long loopStartTime = 0;
unsigned long hangDetectionTime = 0;
unsigned long runtimeStateSaveTimerLast = 0;
...
int filteredEventIndices[200];

// ============ ПИНЫ И ТАЙМЕРЫ НАСОСОВ ============

unsigned long emergencyPumpTimerStart = 0;
unsigned long emergencyPump2TimerStart = 0;
...
/// Функции синхронизации и тайм-ауты (в loop)
void loopESP32();
 ...
 * 4. Инициализация I2C шины
 * 5. Инициализация файловой системы SPIFFS (UI only)
 * 6. Загрузка настроек из Preferences
 * 7. Инициализация RTC часов
 * 8. Инициализация Watchdog
 * 9. Загрузка журнала событий (из SD при доступе, иначе SPIFFS)
 ...
 * 3. Обработка энкодера
 * 4. Управление подсветкой и дисплеем
 * 5. Обновление аварийного мониторинга
 * 6. Обновление таймеров и их обратного отсчета
 * 7. Проверка и очистка старых логов
